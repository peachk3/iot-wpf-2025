    ...
    public partial class BookGenreViewModel : ObservableObject
    {
        ...
        private async void LoadGridFromDb()
        {
            try
            {
                ...
                using (MySqlConnection conn = new MySqlConnection(Common.CONNSTR)) // Common.cs 공통화 작업
                {
                    conn.Open();
        ...
        public async void DelData()
        {
            ...
            try
            {
                ...
                using (MySqlConnection conn = new MySqlConnection(Common.CONNSTR))
                {
                    ...
                    int resultCnt = cmd.ExecuteNonQuery(); // 1건 삭제가 되면 resultCnt = 1, 안 지워지면 resultCnt = 0
                    ...
                    {
                        Common.LOGGER.Info("책장르 데이터 삭제 실패");
